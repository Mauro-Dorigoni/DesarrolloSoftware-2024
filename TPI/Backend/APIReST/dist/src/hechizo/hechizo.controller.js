...
async function getAvailableForVisualizacion(req, res) {
    try {
        ...
        const hechizosFiltrados = hechizos.filter(h => {
            ...
            const tieneSolicitudAprobadaOPendiente = h.solicitudes.getItems().some(s => s.mago && s.mago.id === magoExistente.id && ['aprobada', 'pendiente_revision'].includes(s.estado ?? ''));
...
async function findPermitedForUser(req, res) {
    try {
        const magoExistente = validateUser(req);
        if (!magoExistente) {
            return res.status(401).json({ message: "Not authenticated" });
